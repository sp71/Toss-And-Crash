...
local group, bg, level, pillar, pillar2, pig, pig2, ground, bird, msg, msg2, tapMsg, scoreMsg, nextLevel
local curLevel = 1
local numThrows = 0
local score = 0
...
function displayObjects()
	bg = display.newImage ("cloud.png", width/2, height/2)
 	bg.alpha = .5
 	pillar = display.newImage("Pillar.png", width*.55, height*.70)
 	pillar2 = display.newImage("Pillar.png", width*.90, height*.70)
 	pig = baconMaker(width*.90, height*.50, height*.9)  
 	pig2 = baconMaker(width*.55, height*.40,height*.6)
